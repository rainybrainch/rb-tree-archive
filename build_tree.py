  ...
  SEEDS.forEach(s=>{{
    ...
    if(isSel||st==="done"){{
      svg.appendChild(el("rect",{{
        x:s._x-4,y:s._y-4,width:NW+8,height:NH+8,rx:ns.nr+3,ry:ns.nr+3,
    ...
    g.appendChild(el("rect",{{x:s._x,y:s._y,width:NW,height:NH,rx:ns.nr,ry:ns.nr,fill,stroke:strk,"stroke-width":isSel?"2.5":String(ns.sw)}}));
    ...
    parts.forEach((part,pi)=>{{
      ...
      g.appendChild(el("text",{{
        ...
        fill:st==="done"?lc.c:"#f0eedd","font-size":String(ns.fs),"font-family":"Noto Serif JP","font-weight":"700"
      }},part));
    }});
